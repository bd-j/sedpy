...
class Filter(object):
    ...
    def get_properties(self):
        ...
        i0 = np.trapezoid(self.transmission * np.log(self.wavelength),
                      np.log(self.wavelength))
        i1 = np.trapezoid(self.transmission,
                      np.log(self.wavelength))
        i2 = np.trapezoid(self.transmission * self.wavelength,
                      self.wavelength)
        i3 = np.trapezoid(self.transmission,
                      self.wavelength)
        ...
        i4 = np.trapezoid(self.transmission *
        ...
        self.effective_width = (2.0 * np.sqrt(2. * np.log(2.)) *
                                self.gauss_width *
                                self.wave_effective)
        # self.norm  = np.trapezoid(transmission,wavelength)
    ...
    def obj_counts_hires(self, sourcewave, sourceflux, sourceflux_unc=0):
        ...
        if True in (newtrans > 0.):
            ...
            counts = np.trapezoid(sourcewave[ind] * newtrans[ind] *
                              sourceflux[..., ind],
    ...
    def obj_counts_lores(self, sourcewave, sourceflux, sourceflux_unc=0):
        ...
        if True in (newflux > 0.):
            counts = np.trapezoid(self.wavelength * self.transmission * newflux,
                              self.wavelength)
            return np.squeeze(counts)
        else:
            return float('NaN')
...
class FilterSet(object):
    ...
    def get_sed_maggies_trapz(self, sourceflux, sourcewave=None):
        ...
        maggies = np.trapezoid(y, x, axis=-1)
        return maggies
...
def Lbol(wave, spec, wave_min=90, wave_max=1e6):
    ...
    return np.trapezoid(spec[..., inds[0]], wave[inds])
