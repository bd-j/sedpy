...
class F99(GenericCurve):
    ...
    def default_pars(self, var = False):
        ...
        p['R_v'] = 3.1 +var*normal(0., 0.6)
        #Optical
        p['ospline_x'] = 1e4/np.array([4100., 4670., 5470., 6000., 12200, 26500, 3e8])
        p['ospline_x'][-1] = 0.
    ...
    def selfupdate(self, var = False):
        ...
        self.pardict['c1'] = 2.02 - 3.007 * self.pardict['c2'] +var*normal(0, 0.3)
        self.pardict['ospline_k'] = np.array([1.208 + 0.0032 * p['R_v'] - 0.00033 * p['R_v']**2,
                                              0.701 + 0.0016 * p['R_v'],
                                              -0.050 + 0.0016 * p['R_v'],
                                              -0.426 + 0.0044 * p['R_v'],
                                              (0.829/3.1 - 1) * p['R_v'],
    ...
    def ecurve(self, x, pardict):
        ...
        spline_k = np.array(self.fm_curve(spline_x[0:2], **pardict).tolist() + 
                             pardict['ospline_k'].tolist())
        optical  = self.spline(x, spline_x =spline_x[::-1], spline_k = spline_k[::-1]) * (x <= 3.8)
        return uv + optical
