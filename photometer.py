...
class Aperture(object):
    ...
    def object_flux(self, shape, image, ivar = None):
        ...
        unc = 0
        if ivar is not None:
            unc = sqrt((fracs/ivar[inds]).sum())
        return (image[inds]*fracs).sum(), fracs.sum(), unc
...
class Background(object):
    ...
    def evaluate(self, image, skypars):
        ...
        value, sdpp = self.skystats(image[inds], **skypars)
...
def circle_frac_quick(xcen = 0, ycen = 0, radius = 1, inner_radius = None, subpixels = 1, **extras):
    ...
    print(type(pix1[0]), type(pix1))
    x = (pix1[0] + start[0]/subpixels)
    y = (pix1[1] + start[1]/subpixels)
    return (x,y), fracs
...
def gaussfit_sky(values, p_thresh = 0.65, **extras):
    ...
    #pdf = hist/dbin
    val_thresh = np.percentiles(values, p_thresh)
    lower = where(cbin < p_thresh)
    ...
    p0 = [hist.max(), values.mean(), values.std()]
    coeff, var_matrix = curve_fit(gauss, cbin, hist, p0=p0)
